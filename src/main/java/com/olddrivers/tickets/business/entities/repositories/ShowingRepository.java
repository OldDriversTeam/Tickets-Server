...
public interface ShowingRepository extends CrudRepository<Showing, String> {
	...
	@Query("select s.id, s.time, s.price, s.room.id, s.room.name from Showing s where s.room.cinema.id = ?1 AND s.date = ?2 AND s.movie.id = ?3")
	List<Object[]> findShowingIdListByCinemaIdAndDateAndMovieId(String cinemaId,String date,String movieId);
}
